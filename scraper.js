...
function asyncwork(week, globalCallback) {
	...
	function updateMenus(callback) {
		...
		eventEmitter.on('scraped', function(message) {

			console.log("number of scrapes is : " + number_of_scrapes);
...
/*
	...
	updateMenus(function() {
		...
		function saveMenu() {
			...
			db.saveDoc(menus.date, menus, function(err, ok) {
				...
				util.puts('-> Saved the menu to couch as ' + menus.date);
	...
	function getMenu() { // getMenu(location, date, meal, callback)
		...
		jsdom.env('http://www.campusdish.com/en-US/CSS/UnivSouthFlorida/LocationsMenus/menuJuniperDining.htm?LocationName=' + locationParam + '&MealID=' + mealParam + '&OrgID=137351&Date=' + dateParam + '&ShowPrice=False&ShowNutrition=True', ['http://code.jquery.com/jquery-1.7.min.js'], function(errors, window) {
			var $ = window.$;
			...
			// });
			number_of_scrapes++;
			eventEmitter.emit('scraped', 'Menu has finished scraping: ' + location + ' - ' + meal + ' - ' + dateParam);
		});
	}
}
